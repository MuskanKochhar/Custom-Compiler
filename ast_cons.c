...
AST* lt(AST* lhs, AST* rhs)     { return make_binexpr(yylineno, BE_LT, lhs, rhs); }
AST* lte(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_LTE, lhs, rhs); }
AST* gt(AST* lhs, AST* rhs)     { return make_binexpr(yylineno, BE_GT, lhs, rhs); }
AST* gte(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_GTE, lhs, rhs); }
AST* eq(AST* lhs, AST* rhs)     { return make_binexpr(yylineno, BE_EQ, lhs, rhs); }
AST* neq(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_NEQ, lhs, rhs); }
AST* add(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_ADD, lhs, rhs); }
AST* sub(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_SUB, lhs, rhs); }
AST* mul(AST* lhs, AST* rhs)    { return make_binexpr(yylineno, BE_MUL, lhs, rhs); }
AST* divide(AST* lhs, AST* rhs) { return make_binexpr(yylineno, BE_DIV, lhs, rhs); }
...
{
    AST* ast = make_ast(yylineno, K_STRING);
    ast->d.str_val_ = str;          // fixed: assign to correct field
    ast->type_ = TYPE_ERR;          // optional: define TYPE_STRING later
    return ast;
}
...
AST* ret(AST *expr)
{
    AST* temp = make_ast(yylineno, AST_RETURN);
...
{
    AST* temp = make_ast(yylineno, AST_WHILE);
...
{
    AST* temp = make_ast(yylineno, AST_IF_THEN);
...
{
    AST* ast = make_ast(yylineno, AST_PRINT);
    ast->d.print_stmt.args_ = args;
    return ast;
}
