...
static void semant_string(AST *node, struct symtab *M, struct symtab *V)
{
    set_type(node, TYPE_STRING);
    set_symtab(node, M, V);
}

// ✅ New: Handle print statements
static void semant_print(AST *node, struct symtab *M, struct symtab *V)
{
    set_symtab(node, M, V);
    semant(node->d.print_stmt.args_, M, V);

    for (struct node *tmp = node->d.print_stmt.args_->head; tmp != NULL; tmp = tmp->next)
    {
        int t = tmp->data->type_;
        if (t != TYPE_INT && t != TYPE_STRING)
        {
            semant_error("Invalid type in print (must be int or string)", tmp->data);
        }
    }
...
static void semant_(AST *node, struct symtab *M, struct symtab *V)
{
    ...
    case K_CALL:
    {
        ...
        AST *f = lookup(M, node->d.call.name_);
        if (!f || f->kind_ != FUNCTION)
        {
            semant_error("Function not declared", node);
            set_type(node, TYPE_ERR);
        }
        else
        {
            // Optional: check argument counts/types
            set_type(node, TYPE_INT); // assuming all functions return int for now
        }
